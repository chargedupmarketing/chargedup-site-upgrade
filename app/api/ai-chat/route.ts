...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '- process.env.CLARIFAI_API_KEY exists:',
      !!process.env.CLARIFAI_API_KEY
    );
    console.log(
      '- process.env.CLARIFAI_API_KEY length:',
      process.env.CLARIFAI_API_KEY?.length || 0
    );
    console.log(
      '- process.env.CLARIFAI_API_KEY first 10 chars:',
      process.env.CLARIFAI_API_KEY?.substring(0, 10) + '...' || 'undefined'
    );
    console.log(
      '- All env vars starting with CLARIFAI:',
      Object.keys(process.env).filter(key => key.startsWith('CLARIFAI'))
    );

    const { message, conversationHistory, userId, userName }: ChatRequest =
      await request.json();
    ...
    const apiKey =
      process.env.CLARIFAI_API_KEY || 'd0a1e2fb8c164b0a909189ee221f886e';
    console.log('API Key check:');
    console.log('- apiKey variable length:', apiKey.length);
    console.log('- apiKey first 10 chars:', apiKey.substring(0, 10) + '...');
    console.log(
      '- apiKey matches fallback:',
      apiKey === 'd0a1e2fb8c164b0a909189ee221f886e'
    );
    console.log(
      '- process.env.CLARIFAI_API_KEY matches apiKey:',
      process.env.CLARIFAI_API_KEY === apiKey
    );

    if (!apiKey || apiKey === 'd0a1e2fb8c164b0a909189ee221f886e') {
      console.log(
        'Error: Using fallback API key - environment variable not loaded!'
      );
      return NextResponse.json(
    ...
    const personalizedSystemPrompt = AI_CONFIG.SYSTEM_PROMPT.replace(
      'the user',
      userName || 'the user'
    );
