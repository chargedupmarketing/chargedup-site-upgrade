...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = [
      'firstName', 'lastName', 'email', 'company', 'service', 
      'budget', 'timeline', 'teamSize', 'currentChallenges', 'goals'
    ]
    ...
    const inquiry: EnterpriseInquiry = {
      id: body.id || `ent_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    ...
    const webhookUrl = process.env.HIGHLEVEL_WEBHOOK_URL || process.env.ENTERPRISE_WEBHOOK_URL
    if (webhookUrl) {
      try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      inquiries: [],
      message: 'Inquiries are currently stored in browser localStorage. Check the admin panel for client-side data.'
    })
