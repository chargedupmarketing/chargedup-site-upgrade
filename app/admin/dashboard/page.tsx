...
export default function AdminDashboard() {
  ...
  const [sortBy, setSortBy] = useState<'timestamp' | 'name' | 'company'>('timestamp')
  ...
  const exportToCSV = () => {
    const headers = ['Date', 'Name', 'Email', 'Company', 'Website', 'Source', 'Services', 'Budget', 'Notes', 'Newsletter']
    const csvContent = [
      headers.join(','),
      ...
      <header className="bg-white/5 border-b border-white/10">
        <div className="container max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
                 ...
                 <p className="text-white/60 text-sm">Welcome back, {adminUser.name} ({adminUser.role})</p>
               </div>
              ...
              <table className="w-full">
                <thead className="bg-white/10">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-white/80 uppercase tracking-wider">Company</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-white/80 uppercase tracking-wider">Services</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-white/80 uppercase tracking-wider">Budget</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-white/80 uppercase tracking-wider">Source</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-white/80 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/10">
                  {sortedLeads.map((lead, index) => (
                    <tr key={lead.id || index} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <div className="text-sm text-white/60">{lead.website}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-white/80">
