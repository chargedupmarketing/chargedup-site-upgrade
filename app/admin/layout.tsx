...
export default function AdminLayout({
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      icon: Building,
      access: ['admin', 'super_admin'],
      description: 'Manage AI Automated Cold DM and AI Marketing Platform inquiries'
    },
    {
      name: 'GoHighLevel',
  ...
  return (
    <div className={containerClass}>
      ...
      {/* Sidebar */}
      <div className={`fixed top-0 bottom-0 left-0 lg:top-1/2 lg:-translate-y-1/2 lg:h-[72vh] lg:left-4 z-50 w-64 ${invertedMode ? 'bg-white/70 border-black/10' : 'bg-white/5 border-white/10'} backdrop-blur-xl border transform transition-transform duration-300 ease-in-out lg:rounded-2xl lg:shadow-2xl lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        {/* User Info */}
        <div className={`px-6 py-4 ${invertedMode ? 'border-b border-black/10' : 'border-b border-white/10'}`}>
          <div className="flex items-center">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${invertedMode ? 'bg-black text-white' : 'bg-gradient-to-br from-cyan-400 to-emerald-400 text-black shadow-lg shadow-cyan-500/20'}`}>
            ...
            <div className="ml-3">
              ...
              <div className="flex items-center">
                <Shield className={`w-3 h-3 mr-1 ${invertedMode ? 'text-emerald-600' : 'text-emerald-400'}`} />
                <span className={`text-xs capitalize ${invertedMode ? 'text-black/60' : 'text-white/60'}`}>{adminUser.role.replace('_', ' ')}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-1">
            {filteredNavigation.map((item) => {
              ...
              return (
                  ...
                  >
                    <Icon className={`mr-3 h-5 w-5 ${isActive ? '' : invertedMode ? 'text-black/50 group-hover:text-black' : 'text-white/50 group-hover:text-white'}`} />
                    <div className="text-left">
                      <p className={`${isActive ? 'font-semibold' : 'font-medium'}`}>
                        {item.name}
                      </p>
                      <p className={`text-xs ${isActive ? 'text-white/80' : invertedMode ? 'text-black/50' : 'text-white/50'}`}>
                        {item.description}
                      </p>
                    </div>
                  </button>
        ...
        {adminUser?.role === 'super_admin' && (
          ...
          <div className="absolute bottom-24 left-0 right-0 px-3">
            ...
            >
              <div className="flex items-center">
                <DollarSign className={`mr-3 h-5 w-5 ${invertedMode ? 'text-black/50 group-hover:text-black' : 'text-white/50 group-hover:text-white'}`} />
                Financial & Business
              </div>
              <ChevronDown 
              ...
              {financialDropdownOpen && (
                <div className={`absolute left-full top-1/2 transform -translate-y-1/2 ml-2 ${invertedMode ? 'bg-white/80 border-black/10' : 'bg-white/5 border-white/10'} backdrop-blur-xl rounded-lg shadow-lg min-w-64 z-50`}>
                  <div className="p-3">
                    <div className="grid grid-cols-3 gap-2">
                      {financialServices.map((service) => {
                        ...
                        return (
                          ...
                          >
                            <Icon className={`mb-1 h-4 w-4 ${invertedMode ? 'text-black/50 group-hover:text-black' : 'text-white/50 group-hover:text-white'}`} />
                            <div className="text-center">
                              {service.name}
                              <ExternalLink className={`ml-1 h-2 w-2 inline ${invertedMode ? 'text-black/40' : 'text-white/40'}`} />
                            </div>
                          </button>
                        )
                      })}
          ...
          <div className="absolute bottom-16 left-0 right-0 p-4">
            <div className={`w-full flex items-center justify-between px-3 py-2 rounded-lg ${invertedMode ? 'bg-black/5' : 'bg-white/10'}`}>
              <div className="flex items-center gap-2 text-sm">
                {invertedMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
      ...
      <div className="lg:ml-[18rem]">
        {/* Top bar */}
        <div className={`sticky top-0 z-30 lg:hidden ${invertedMode ? 'bg-white/80 border-b border-black/10' : 'bg-white/5 border-b border-white/10 backdrop-blur-xl'}`}>
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${invertedMode ? 'bg-black text-white' : 'bg-gradient-to-br from-cyan-400 to-emerald-400 text-black'}`}>
        ...
        {/* Page content */}
        <main className={`${invertedMode ? 'filter invert hue-rotate-180' : ''}`}>
