...
export default function EnterpriseInquiries() {
  ...
  const [activeTab, setActiveTab] = useState<
    'ai-cold-dm' | 'ai-marketing-platform' | 'all'
  >('all');
  ...
  const [selectedInquiry, setSelectedInquiry] =
    useState<EnterpriseInquiry | null>(null);
  ...
  const filteredInquiries = inquiries.filter(inquiry => {
    ...
    const matchesPriority =
      !filterPriority || inquiry.priority === filterPriority;
  ...
  const copySelectedToClipboard = async () => {
    ...
    const lines = selected.map(
      i =>
        `${i.firstName} ${i.lastName} <${i.email}> — ${i.company} — ${getServiceName(i.service)} — ${i.budget}`
    );
    try {
  ...
  const shareSelectedViaEmail = () => {
    ...
    const subject = encodeURIComponent(
      `Enterprise inquiries (${selected.length} selected)`
    );
    const bodyLines = selected
      .slice(0, 25)
      .map(
        i =>
          `• ${i.firstName} ${i.lastName} — ${i.company} — ${getServiceName(i.service)} — ${i.budget}`
      );
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">
              Enterprise Service Inquiries
            </h1>
            <p className="text-white/60">
              Manage high-value inquiries for AI Automated Cold DM and AI
        ...
        <div className="rounded-2xl p-6 border border-white/10 bg-white/5 backdrop-blur-sm shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06)]">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-white">
                {
                  inquiries.filter(i => i.service === 'ai-marketing-platform')
                    .length
                }
              </p>
            </div>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              ...
              >
                ...
                <option value="ai-marketing-platform">
                  AI Marketing Platform
                </option>
              </select>
            </div>
            ...
            <div className="sticky top-0 z-10 -mt-2 mb-4 bg-white/10 border border-white/20 rounded-xl backdrop-blur-sm px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 text-sm rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 text-white"
                >
                  Download CSV
                </button>
                <button
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-white/80"
                >
                  Clear
                </button>
              </div>
          ...
          <div className="space-y-4">
            ...
            {filteredInquiries.map(inquiry => (
              <div
                key={inquiry.id}
                className="rounded-2xl p-6 border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset ring-white/10 ${getStatusColor(inquiry.status)}`}
                      >
                        {inquiry.status}
                      </span>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset ring-white/10 ${getPriorityColor(inquiry.priority)}`}
                      >
                        {inquiry.priority}
                      </span>
                    </div>
                    ...
                    {inquiry.notes && (
                      <div className="mb-4">
                        ...
                        <div className="text-white text-sm bg-white/5 border border-white/10 p-3 rounded-xl">
                          {inquiry.notes}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-4 text-xs text-white/60">
                      ...
                      {inquiry.lastContact && (
                        <span>
                          Last Contact:{' '}
                          {new Date(inquiry.lastContact).toLocaleDateString()}
                        </span>
                      )}
                      {inquiry.assignedTo && (
